#!/usr/bin/env python3
"""
Chunked import player statistics for all years with maximum performance
"""
import asyncio
import sys
import pandas as pd
...
class ChunkedPlayerStatsImporter:
    def __init__(self, db_manager: DatabaseManager, team_mapping: Dict[str, str], player_mapping: Dict[str, str]):
        ...
        self.batch_size = 5000  # Process 5000 records at a time
        self.chunk_size = 50000  # Read 50k rows at a time from CSV
    ...
    async def import_all_stats_chunked(self, csv_path: str, start_year: int = 2014, end_year: int = 2024):
        """Import player statistics using chunked CSV reading for maximum performance"""
        print(f"🚀 Chunked importing player stats for {start_year}-{end_year} seasons from {csv_path}")
        ...
        # Read CSV in chunks
        chunk_iter = pd.read_csv(csv_path, chunksize=self.chunk_size, low_memory=False)
        ...
        for chunk_num, chunk_df in enumerate(chunk_iter):
            ...
            chunk_df['gameDate'] = pd.to_datetime(chunk_df['gameDate'])
            chunk_df['year'] = chunk_df['gameDate'].dt.year
            filtered_chunk = chunk_df[(chunk_df['year'] >= start_year) & (chunk_df['year'] <= end_year)].copy()
            
            if len(filtered_chunk) == 0:
                continue
            
            # Add season column
            filtered_chunk['season'] = filtered_chunk['gameDate'].dt.year
            filtered_chunk['season'] = filtered_chunk['season'].astype(str) + '-' + (filtered_chunk['season'] + 1).astype(str).str[2:]
            
            # Group by player and season
            grouped = filtered_chunk.groupby(['firstName', 'lastName', 'season'])
            ...
            print(f"📊 Chunk {chunk_num + 1} complete: {processed:,} records processed, {rate:.0f} records/sec total")
        
        total_time = time.time() - start_time
        print(f"✅ Chunked player stats import completed!")
        print(f"📊 Total processed: {total_processed:,} player-season combinations")
...
async def main():
    """Import all player statistics using chunked processing"""
    print("🚀 Starting Chunked Player Statistics Import (All Years)...")
    ...
    try:
        ...
        # Create chunked importer
        importer = ChunkedPlayerStatsImporter(db_manager, team_mapping, player_mapping)
        
        # Import all player stats (2014-2024 seasons)
        await importer.import_all_stats_chunked("data/PlayerStatistics.csv", start_year=2014, end_year=2024)
        
    except Exception as e:
        print(f"❌ Error during chunked player stats import: {e}")
        raise
    finally:
