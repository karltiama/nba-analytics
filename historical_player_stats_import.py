#!/usr/bin/env python3
"""
Import player statistics for historical years (2019-2023)
"""
import asyncio
import sys
import pandas as pd
...
class HistoricalPlayerStatsImporter:
    ...
    async def import_historical_stats(self, csv_path: str, start_year: int = 2019, end_year: int = 2023):
        """Import player statistics for historical years"""
        print(f"🚀 Importing player stats for {start_year}-{end_year} seasons from {csv_path}")
        ...
        historical_df = df[(df['year'] >= start_year) & (df['year'] <= end_year)].copy()
        print(f"📊 Records for {start_year}-{end_year} seasons: {len(historical_df):,}")
        ...
        # Add season column
        historical_df['season'] = historical_df['gameDate'].dt.year
        historical_df['season'] = historical_df['season'].astype(str) + '-' + (historical_df['season'] + 1).astype(str).str[2:]
        
        # Group by player and season
        grouped = historical_df.groupby(['firstName', 'lastName', 'season'])
        ...
        for (first_name, last_name, season), group in grouped:
            try:
                ...
                if not player_id:
                    if processed % 1000 == 0:  # Only print every 1000 to avoid spam
        ...
        print(f"✅ Historical player stats import completed!")
...
async def main():
    """Import historical player statistics"""
    print("🚀 Starting Historical Player Statistics Import (2019-2023)...")
    ...
    try:
        ...
        # Create historical importer
        importer = HistoricalPlayerStatsImporter(db_manager, team_mapping, player_mapping)
        
        # Import historical player stats (2019-2023 seasons)
        await importer.import_historical_stats("data/PlayerStatistics.csv", start_year=2019, end_year=2023)
        
    except Exception as e:
        print(f"❌ Error during historical player stats import: {e}")
        raise
    finally:
